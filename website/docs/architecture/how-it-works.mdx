...
The installation flow of skills-package-manager can be understood as three stages: resolve, materialize, and link. Remote git mirrors, npm metadata, and npm tarballs are cached in a user-level cache directory so repeated installs can reuse previously fetched data.
...
- Git sources are checked out from a cached mirror into a temporary worktree
- npm sources are extracted from cached tarballs using temporary staging directories
- The cache location can be overridden with `SPM_CACHE_DIR`
