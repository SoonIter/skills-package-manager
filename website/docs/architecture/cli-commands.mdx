...
1. Reuse or create a cached git mirror for the target repository
...
3. **Sync lockfile** — Resolve each skill in the manifest to produce the lockfile entries for this install
   ...
   - Cached git mirrors, npm metadata, and npm tarballs are reused across commands and projects
4. **Prune** — Remove skills that are no longer in the manifest
5. **Fetch** — Materialize skills to `installDir` from local paths, cached git worktrees, or cached tarballs
6. **Link** — Create symlinks in `linkTargets`
7. **Write lockfile** — Save updated `skills-lock.yaml`

Set `SPM_CACHE_DIR` to override the default global cache location.
