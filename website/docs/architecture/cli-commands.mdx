...
1. **Read manifest** — Load `skills.json` to get the list of skills to install
   - If `selfSkill` is enabled and exactly one repo-authored skill exists under `skills/` or `.github/skills/`, expand the effective manifest with that local `link:` skill
...
- Skips local `link:` skills, including auto-discovered self skills
