...
3. **Sync lockfile** — For each skill in manifest:
   - If specifier changed or skill not in lockfile: resolve new version (may involve git network requests)
   - Otherwise: use existing lockfile entry
