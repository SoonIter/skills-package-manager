...
- `selfSkill`: whether to auto-install the bundled `skills-package-manager-cli` skill for help with `skills.json`, `skills-lock.yaml`, and `spm`. This helper skill is not written to `skills-lock.yaml`. Defaults to `false`
